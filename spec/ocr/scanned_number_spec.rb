...
describe OCR::ScannedNumber do
  ...
  context "with an unrecognized digit" do
    ...
    When(:number) { OCR::ScannedNumber.new(lines) }
  ...
  describe "illformed numbers" do
    When(:result) { OCR::ScannedNumber.new(lines) }
    ...
    context "with illegal characters" do
      ...
      Then { result.should have_failed(OCR::IllformedScannedNumberError, /illegal character/i)  }
    end
    ...
    context "with lines lengths not divisible by 3" do
      ...
      Then { result.should have_failed(OCR::IllformedScannedNumberError, /length.*(3|three)/i)  }
    end
    ...
    context "with missing lines" do
      ...
      Then { result.should have_failed(OCR::IllformedScannedNumberError, /(3|three) lines/i)  }
    end
  end

  describe "creating numbers from digit strings" do
    When(:number) { OCR::ScannedNumber.from_digits(digits) }
